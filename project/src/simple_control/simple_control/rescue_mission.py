...
class RescueMission(Node):
    ...
    def plan_path(self):
        ...
        # Log path in map coordinates
        cells_pretty = "\n".join([f"   {p}" for p in self.current_path_cells])
        self.get_logger().info(
            f"[PLAN] HELLO HERE Path (map coords, {len(self.current_path_cells)} steps):\n{cells_pretty}"
        )

        # Log path in world coordinates
        world_pretty = "\n".join([f"   ({wx:.2f}, {wy:.2f})" for (wx, wy) in self.current_path_world])
        self.get_logger().info(
            f"[PLAN] HI HERE Path (world coords):\n{world_pretty}"
        )
